...
IFaceFrameReader*                   m_pFaceFrameReader = NULL;
...
// Face
std::thread               m_tFaceThread;
std::mutex                m_mFaceReaderMutex;
bool                      m_bFaceThreadRunning = false;
Napi::ThreadSafeFunction  m_tsfnFace;
std::mutex                m_mFaceThreadJoinedMutex;
...
Napi::Value MethodOpenFaceFrameReader(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();

  if (m_bFaceThreadRunning) {
    Napi::TypeError::New(env, "Face thread already running")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  HRESULT hr;
  IFaceFrameSource* pFaceFrameSource = NULL;
  hr = m_pFaceFrameReader->get_FaceFrameSource(&pFaceFrameSource);
  if (SUCCEEDED(hr))
  {
  ...
  if (SUCCEEDED(hr))
  {
    ...
    m_tsfnFace = Napi::ThreadSafeFunction::New(
      env,
      info[0].As<Napi::Function>(),     // JavaScript function called asynchronously
      "Kinect2 Face Listener",              // Name
      1,                                // 1 call in queue
      1,                                // Only one thread will use this initially
      []( Napi::Env ) {                 // Finalizer used to clean threads up
        m_tFaceThread.join();
        m_mFaceThreadJoinedMutex.unlock();
        printf("[kinect2.cc] Face thread joined\n");
      });

    m_mFaceThreadJoinedMutex.lock();
    m_tFaceThread = std::thread( [] {
      
      auto callback = []( Napi::Env env, Napi::Function jsCallback ) {
        if (!m_bFaceThreadRunning) {
          return;
        }
        m_mFaceReaderMutex.lock();

        Napi::Object v8FaceResult = getV8FaceFrame(env, &m_jsFaceFrame);

        jsCallback.Call( { v8FaceResult } );
        m_mFaceReaderMutex.unlock();
      };

      while(m_bFaceThreadRunning)
      {
        ...
        if (SUCCEEDED(hr))
        {
          processFaceFrameData(pFaceFrame);
        }
        if (!m_bFaceThreadRunning)
        {
          m_mFaceReaderMutex.unlock();
        ...
        if (SUCCEEDED(hr))
        {
          napi_status status = m_tsfnFace.BlockingCall( callback );
          if ( status != napi_ok )
          {
            break;
          }
